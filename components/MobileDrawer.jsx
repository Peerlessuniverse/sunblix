'use client';

export default function MobileDrawer({ isOpen, onClose, onOpenQuote }) {
  const handleNavClick = (milestone) => {
    onClose();
    if (typeof window !== 'undefined') {
      const event = new CustomEvent('sunblix:scrollto', { detail: { target: milestone } });
      window.dispatchEvent(event);
    }
  };

  const handleQuoteClick = (e) => {
    e.preventDefault();
    onClose();
    onOpenQuote();
  };

  return (
    <>
      <div
        className={`mobile-drawer-backdrop ${isOpen ? 'is-active' : ''}`}
        id="mobileDrawerBackdrop"
        aria-hidden={!isOpen}
        onClick={onClose}
        style={{
          visibility: isOpen ? 'visible' : 'hidden',
          opacity: isOpen ? 1 : 0,
          transition: 'all 0.3s ease',
          pointerEvents: isOpen ? 'auto' : 'none',
        }}
      />
      <aside
        className={`mobile-drawer ${isOpen ? 'is-open' : ''}`}
        id="mobileDrawer"
        aria-label="Mobile Navigation"
        aria-hidden={!isOpen}
        style={{
          transform: isOpen ? 'translateX(0)' : 'translateX(100%)',
          transition: 'transform 0.35s cubic-bezier(0.16, 1, 0.3, 1)',
        }}
      >
        <div className="drawer-header">
          <a
            href="#home"
            className="drawer-logo-link"
            onClick={(e) => {
              e.preventDefault();
              onClose();
              if (typeof window !== 'undefined') {
                window.dispatchEvent(new CustomEvent('sunblix:refresh'));
              }
            }}
          >
            <img src="/asset/sublixlogo.svg" alt="SUNBLIX" className="drawer-logo" />
          </a>
          <button
            type="button"
            className="drawer-close-btn"
            aria-label="Close menu"
            onClick={onClose}
          >
            ✕
          </button>
        </div>

        <nav className="drawer-nav">
          <a
            href="#home"
            className="drawer-link"
            onClick={(e) => {
              e.preventDefault();
              handleNavClick('#home');
            }}
          >
            <span className="link-label">Home</span>
            <span className="link-arrow">→</span>
          </a>
          <a
            href="#solutions"
            className="drawer-link"
            onClick={(e) => {
              e.preventDefault();
              handleNavClick('#solutions');
            }}
          >
            <span className="link-label">Solusi Energi PLTS</span>
            <span className="link-arrow">→</span>
          </a>
          <a
            href="#products"
            className="drawer-link"
            onClick={(e) => {
              e.preventDefault();
              handleNavClick('#products');
            }}
          >
            <span className="link-label">Pilihan Paket Residential</span>
            <span className="link-arrow">→</span>
          </a>
          <a
            href="#how-it-works"
            className="drawer-link"
            onClick={(e) => {
              e.preventDefault();
              handleNavClick('#how-it-works');
            }}
          >
            <span className="link-label">Cara Kerja Aliran Daya</span>
            <span className="link-arrow">→</span>
          </a>
          <a
            href="#why-sunblix"
            className="drawer-link"
            onClick={(e) => {
              e.preventDefault();
              handleNavClick('#why-sunblix');
            }}
          >
            <span className="link-label">Keunggulan & Tim Teknisi</span>
            <span className="link-arrow">→</span>
          </a>
          <a
            href="#projects"
            className="drawer-link"
            onClick={(e) => {
              e.preventDefault();
              handleNavClick('#projects');
            }}
          >
            <span className="link-label">Proyek & Peta 19+ Kota</span>
            <span className="link-arrow">→</span>
          </a>
          <a
            href="#projects"
            className="drawer-link highlight-calc"
            onClick={(e) => {
              e.preventDefault();
              handleNavClick('#projects');
            }}
          >
            <span className="link-label">⚡ Kalkulator Hemat Listrik</span>
            <span className="link-arrow">→</span>
          </a>
        </nav>

        <div className="drawer-actions">
          <button
            type="button"
            className="drawer-btn-quote"
            onClick={handleQuoteClick}
            style={{ width: '100%', border: 'none', cursor: 'pointer', textAlign: 'center' }}
          >
            <span>Dapatkan Penawaran Resmi</span>
            <span className="btn-icon">→</span>
          </button>
          <a
            href="https://wa.me/6281112345678?text=Halo%20SUNBLIX,%20saya%20ingin%20konsultasi%20pemasangan%20solar%20panel%20PLTS."
            target="_blank"
            rel="noopener noreferrer"
            className="drawer-btn-wa"
          >
            <svg viewBox="0 0 24 24" width="20" height="20" fill="currentColor">
              <path d="M12.04 2C6.58 2 2.13 6.45 2.13 11.91C2.13 13.66 2.59 15.36 3.45 16.86L2.05 22L7.3 20.62C8.75 21.41 10.38 21.83 12.04 21.83C17.5 21.83 21.95 17.38 21.95 11.92C21.95 9.27 20.92 6.78 19.05 4.91C17.18 3.03 14.69 2 12.04 2M12.05 3.67C14.25 3.67 16.31 4.53 17.87 6.09C19.42 7.65 20.28 9.72 20.28 11.92C20.28 16.46 16.58 20.15 12.04 20.15C10.56 20.15 9.11 19.76 7.85 19L7.55 18.83L4.43 19.65L5.26 16.61L5.06 16.29C4.24 15 3.8 13.47 3.8 11.91C3.81 7.37 7.5 3.67 12.05 3.67M9.53 7.34C9.36 7.34 9.08 7.41 8.84 7.67C8.6 7.93 7.92 8.57 7.92 9.87C7.92 11.17 8.87 12.43 9 12.6C9.13 12.77 10.87 15.46 13.53 16.61C14.16 16.88 14.66 17.05 15.04 17.17C15.68 17.37 16.26 17.35 16.72 17.28C17.24 17.2 18.31 16.63 18.53 16C18.75 15.37 18.75 14.83 18.69 14.72C18.63 14.61 18.46 14.54 18.2 14.41C17.94 14.28 16.67 13.65 16.44 13.57C16.21 13.48 16.04 13.44 15.87 13.7C15.7 13.96 15.22 14.54 15.07 14.71C14.92 14.88 14.77 14.9 14.51 14.77C14.25 14.65 13.42 14.37 12.43 13.49C11.66 12.8 11.14 11.95 11 11.7C10.85 11.45 11 11.31 11.12 11.18C11.24 11.07 11.38 10.89 11.5 10.74C11.63 10.6 11.67 10.49 11.76 10.32C11.85 10.15 11.8 10 11.74 9.87C11.67 9.75 11.17 8.52 10.96 8.01C10.76 7.51 10.55 7.58 10.39 7.57C10.24 7.56 10.07 7.56 9.9 7.56L9.53 7.34Z" />
            </svg>
            <span>Konsultasi WhatsApp Cepat</span>
          </a>
        </div>

        <div className="drawer-footer-info">
          <span>PT SUNBLIX ENERGI INDONESIA</span>
          <p>Mitra Terpercaya Solusi PLTS Rooftop Indonesia</p>
        </div>
      </aside>
    </>
  );
}
